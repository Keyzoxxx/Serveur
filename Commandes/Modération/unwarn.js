...
const { ChannelLogsUnwarn } = require("../../config");
      ...
      .setDescription(`❌ \*Vous ne pouvez pas interagir avec cette personne !\*`)
        ...
        db.query(`SELECT * FROM warns WHERE userID = ${user.id} AND guildID = ${message.guild.id}`, async (err, req) => {
          ...
          try {
            ...
            let ChannelLogsUnwarns = bot.channels.cache.get(ChannelLogsUnwarn)

            let EmbedUnwarn = new Discord.MessageEmbed()
            .setColor(bot.color)
            .setTitle("Suppression avertissement ")
            .setThumbnail('https://images.emojiterra.com/google/android-11/512px/26a0.png')
            .setDescription(`***Utilisateur :*** *${user}*\n***Modérateur :*** *${message.user === undefined ? message.author : message.user}*\n***Date :*** *<t:${Math.floor(message.createdAt / 1000)}:F>*\n***Raison :*** *${reason}.*`)
            .setTimestamp()
            .setFooter(`${bot.user.username}`, bot.user.displayAvatarURL({dynamic: true}))    
        
            ChannelLogsUnwarns.send({embeds: [EmbedUnwarn]})
        })
    }
})
